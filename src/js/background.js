...
function getSpeedDialId() {
  browser.bookmarks
    .search({ title: 'Speed Dial', url: undefined })
    .then(result => {
...
function removeBookmark(id, bookmarkInfo) {
  if (
    bookmarkInfo.url &&
    (bookmarkInfo.parentId === speedDialId ||
      folderIds.indexOf(bookmarkInfo.parentId) !== -1)
  ) {
...
function changeBookmark(id, info) {
  ...
  browser.bookmarks.get(id).then(bookmark => {
    ...
    if (
      bookmark[0].parentId === speedDialId ||
      folderIds.indexOf(bookmark[0].parentId) !== -1
    ) {
      if (bookmark[0].url) {
